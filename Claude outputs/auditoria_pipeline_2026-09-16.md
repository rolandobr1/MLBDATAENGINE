...
## 12. "La extracción tardó bastante" — un bug de re-disparo confirmado y corregido, y una sospecha seria (ahora confirmada — ver sección 13) (16 de septiembre, misma sesión)
...
## 13. CONFIRMADO: crash real por falta de memoria (Node "heap out of memory", exit 134) — arreglo de fondo aplicado (16 de septiembre, sesión de seguimiento)

Me pasaste el dashboard de Render y confirmé la sospecha de la sección 12.2 con evidencia directa, no ya una hipótesis.

**Confirmación en la pestaña "Events":** dos eventos consecutivos, ambos con timestamp 16 de septiembre 4:53 PM — *"Instance failed: rmb6z — Exited with status 134 while running your code"*, seguido inmediatamente por *"Service recovered — Recently failed instances are now reporting healthy"*. El exit code 134 es la firma exacta de un `SIGABRT` (128+6) — coincide con un crash de memoria de Node, no con un timeout ni un error de red. Esto ocurrió ~5.6 minutos después de arrancar el proceso, que es justo el tiempo que toma un harvest completo de 15 partidos.

**Confirmación en la pestaña "Logs":** justo antes del reinicio, la salida muestra el patrón exacto de un crash de V8 por memoria — varias líneas `Mark-Compact ... MB, pooled: ... allocation failure; scavenge might not succeed` (el recolector de basura de Node peleando por liberar espacio antes de rendirse, con el techo de heap rondando 253-258MB), después el bloque `<--- Last few GCs --->` con el stack trace nativo, y finalmente `Aborted (core dumped)`. Es un "JavaScript heap out of memory" real y sin ambigüedad, típico de un plan de Render con RAM limitada — confirmé por separado en el propio dashboard que el servicio corre en el plan **Free** ("Node", "Free", con el aviso de que la instancia se duerme por inactividad), que trae poca RAM.

**Causa de raíz encontrada en el código, no solo el síntoma general de "el archivo es grande":** el harvest tiene un patrón de "checkpoint" — guardar el progreso después de CADA partido, por si el proceso se interrumpe a medio camino — y ese checkpoint hacía `readGamesDB()` (lee y hace `JSON.parse` de TODO `mlb_database.json`, ~213MB) seguido de `writeGamesDB()` (hace `JSON.stringify` y reescribe TODO el archivo) — dos veces por partido, no una vez por día. Para tus 15 partidos de hoy, eso son ~15 lecturas completas + ~15 escrituras completas de un archivo de 213MB solo para persistir el avance de UN día, mientras en paralelo corren los subprocesos de Python (PyBaseball, arsenal) que también consumen memoria. Ese patrón de "release-then-reallocate 213MB, quince veces seguidas, bajo presión de memoria ya existente" es precisamente lo que agota el heap y termina en el abort que se ve en los logs.

**Elegiste la opción de fondo (no solo subir de plan en Render) y ya está implementada:** se reemplazó `mlb_database.json` (un solo archivo) por una carpeta `games_db/` con un archivo JSON chico por fecha (`games_db/2026-09-16.json`, por ejemplo) más un índice liviano (`games_db/_index.json`) que solo guarda, por fecha, cuántos partidos tiene y sus IDs — sin el contenido completo. `readGamesDB()`/`writeGamesDB()` (las funciones que usa el resto del código, sin que haya que tocar los ~30 sitios donde se llaman) ahora cargan y guardan solo la fecha puntual que de verdad se toca, nunca el historial completo — el checkpoint de cada partido pasa de leer/escribir 213MB a leer/escribir un archivo de unos pocos KB. También se corrigieron dos lugares que habrían anulado esta mejora sin querer: la fusión de datos que llegan de Firestore (`mergeGamesIntoLocalDB`) copiaba la base entera antes de tocarla, y el chequeo de "¿este game_id ya existe bajo otra fecha?" al final de cada harvest recorría el contenido completo de las 200+ fechas del historial por cada partido nuevo — ambos se reescribieron para usar el índice liviano en vez de cargar contenido.

**Migración automática:** la primera vez que arranque el server con este cambio y encuentre tu `mlb_database.json` local de 213MB, lo migra una sola vez a `games_db/` (tarda unos segundos) y deja el original renombrado como `mlb_database.json.migrated` por si hace falta revisar algo — no se borra nada. En Render esto en la práctica no debería ni activarse, porque el disco ahí es efímero (se borra en cada redeploy) y arranca vacío, restaurándose desde Firestore.

Verificado con `tsc --noEmit` (mismo baseline de 40 errores preexistentes, ninguno nuevo en `server.ts`), 39/39 tests, un chequeo de bundle con esbuild, y además un test de comportamiento aparte (12 casos) que ejercita directamente la lógica nueva del Proxy contra un directorio temporal real en disco — confirma que: escribir una fecha no toca ni lee ninguna otra fecha; `Object.keys`/`for...in`/`hasOwnProperty` no cargan contenido (siguen siendo baratos); una fecha guardada con 0 partidos se sigue reconociendo como "ya extraída" sin crear un archivo vacío en disco; la fusión desde Firestore y el chequeo de duplicados solo tocan las fechas que de verdad necesitan; y la migración del archivo legado reproduce el mismo conteo de partidos por fecha y es segura de correr dos veces. De paso, ese mismo test encontró y permitió corregir un bug real en la primera versión de este cambio (una clave de caché mal indexada que habría hecho que, en la práctica, siguiera reescribiendo más de lo necesario) — quedó arreglado antes de entregarte el código.

**Pendiente de tu parte:** hacer `git add`/`commit`/`push` de `server.ts` y `.gitignore` para que Render lo despliegue, y luego repetir una extracción completa de un día con muchos partidos para confirmar en los logs de Render que ya no vuelve a aparecer el patrón de heap-out-of-memory. Si de todos modos el plan Free se queda corto de RAM por otras razones (los subprocesos de Python en paralelo, por ejemplo), ese es un problema aparte de éste — este cambio elimina la causa que sí pude confirmar con evidencia directa, pero no descarta que aparezca otra bajo carga suficiente.

---

## 14. Hallazgo nuevo, sin relación con la memoria: las reglas de Firestore de la sección 10.1 probablemente nunca se desplegaron de verdad (16 de septiembre, misma sesión)

Revisando los logs de Render alrededor del crash de la sección 13, encontré esto justo después del reinicio automático: *"[Firestore] Error leyendo rango 2026-09-13..2026-09-16: [FirebaseError: Missing or insufficient permissions.] { code: 'permission-denied', ... }"* — en `loadGamesByDateRangeFromFirestore`, la función que usa `runStartupFirestoreSync` para reconstruir el disco local tras cada arranque en frío.

Esto es exactamente el mismo tipo de error que documenté en la sección 10.1 (permisos faltantes en `games/{gameId}/snapshots/{snapshotId}` y `metadata/{docId}`) — y la explicación más probable es que **nunca llegaste a desplegar el archivo `firestore.rules` que te entregué en esa sesión.** Es un paso fácil de pasar por alto porque no es intuitivo: `git push` (que Render despliega automáticamente) solo actualiza el CÓDIGO de la app — las reglas de seguridad de Firestore viven en la infraestructura de Firebase, separada de Render, y necesitan su propio paso: correr `firebase deploy --only firestore:rules` con el Firebase CLI, o directamente pegar el contenido de `firestore.rules` en la pestaña "Rules" de la consola de Firebase (console.firebase.google.com → tu proyecto → Firestore Database → Rules) y publicar ahí.

Si en efecto no se desplegó, esto explica por qué varias cosas que ya quedaron "corregidas" en el código puede que sigan sin funcionar del todo en producción — no por un bug nuevo, sino porque el permiso que las habilita nunca llegó a activarse del lado de Firebase.

**Pendiente de tu parte:** desplegar `firestore.rules` por cualquiera de las dos vías de arriba. Avisame cuando lo hagas y puedo volver a probar la extracción en vivo para confirmar que ese error de `permission-denied` ya no aparece.
